...
-- Sora moveset all-in-one controller v15 Counterattack fix, revision 4.
--
-- COUNTERATTACK-ONLY ARCHITECTURE
...
--   is stored in newly appended record 0x76, so neither motion ever overwrites
--   the other at runtime. No loaded animation record is rewritten or swapped.
--
-- REQUIRED MSET
--   xa_ex_0010_SoraComboVisuals_v19_V15_NATIVE_COUNTERATTACK.mset
--   SHA-256: fde586cc1323134c15a6b23dd6099f144ab13ad0bc1da1de74f2d88c5317cbdd
--
-- SCOPE
...
--
-- REQUIRED MSET (v19; v15 plus permanent native D5 and appended F7)
--   xa_ex_0010_SoraComboVisuals_v19_V15_NATIVE_COUNTERATTACK.mset
--   SHA-256: fde586cc1323134c15a6b23dd6099f144ab13ad0bc1da1de74f2d88c5317cbdd
--
-- LAYOUT
...
--   Replacement Ragnarok F7 permanently occupies appended record 0x76.
...
-- ========================================================================
-- VERIFIED STEAM ADDRESSES AND V19 APPENDED-RECORD LAYOUT
-- ========================================================================
...
local SLOT_RAGNAROK_CONTAINER = 0x0076
local SLOT_CC_CONTAINER = 0x0003

-- Physical-record offsets relative to the canonical, never-patched slot 0x65.
-- Existing records retain their original v15 addresses. Appended record 0x76
-- begins at file offset 0x23E3A0 in the v19 MSET.
local SLOT_DELTA_FROM_65 = {
    ...
    [SLOT_DC] = 0x6AFD0,
    [SLOT_RAGNAROK_CONTAINER] = 0x70E90,
}
...
local function validateV19Signature(expected)
    ...
    if ragnarok == 0 or rippleGuard == 0 or zantRoll == 0 then
        return false, "v19 signature pointers could not be resolved"
    end

    -- Appended record 0x76 is the exact v15 F7 container and keeps F7's own
    -- empty trigger terminator rather than CE's tail.
    if unsigned32(ReadInt(ragnarok + 0x9CA0, true)) ~= 0xFFFFFFFF then
        return false, "appended slot 0x76 does not contain the v15 F7 trigger"
    end
...
local function inspectV19Layout(sora, allowTemporaryRoutes, allowPermanentRoutes)
    ...
    local pointerRagnarok = expected[SLOT_RAGNAROK_CONTAINER]
    for slot, expectedPointer in pairs(expected) do
        ...
        then
            return nil, string.format(
                "active MSET does not match v19 at slot 0x%02X "
    ...
    local signatureValid, signatureError = validateV19Signature(expected)
...
local function cleanAndApplyRoutes(sora)
    local layout, layoutError = inspectV19Layout(sora, true, true)
    if layout == nil then
        return false, layoutError
    end

    local pointer65 = layout.expected[SLOT_CB]
    local pointerRagnarok = layout.expected[SLOT_RAGNAROK_CONTAINER]
    local cleaned = false
    ...
    -- CE is a permanent v19 route; only CD can be a stale temporary route.
    for _, slot in ipairs({ SLOT_CD }) do
        local current = readMotionPointer(layout.pointerArray, slot)
        if motionPointersEqual(current, pointerRagnarok)
            and not motionPointersEqual(current, layout.expected[slot])
        then
            writeMotionPointer(layout.pointerArray, slot, layout.expected[slot])
            cleaned = true
    ...
    local cleanLayout, cleanError = inspectV19Layout(sora, false, true)
    ...
    if not routesReadyAnnounced then
        ...
        log("READY: v19 appended-record routing is active; native D5 and Ragnarok F7 are separate.")
    end
    return true, nil
end
...
local function beginEntry(entry, sora, animationFrame)
    local layout, layoutError = inspectV19Layout(sora, false, true)
    if layout == nil then
        enabled = false
        ...
        if currentArray ~= permanentPointerArray then
            ...
            if not routed then
                if routeError ~= lastRouteError then
                    detail("Main v19 routing deferred: " .. routeError)
...
local function moduleInit()
    ...
    if sora ~= nil and sora ~= 0 then
        ...
        if not routed then
            ...
            -- exact v19 layout is checked again when the main bank returns.
            detail("Initial v19 routing deferred: " .. routeError)
            lastRouteError = routeError
        end
    else
        previousSora = 0
    ...
    log("Required asset is v19: v15 RAID_DISTANCE_150 with native D5 restored and Ragnarok appended at record 0x76.")
    ...
    log("COUNTERATTACK STORAGE FIX: native D5 is permanent at record 0x6F; replacement Ragnarok F7 is permanent at appended record 0x76.")
    log("Every pre-existing v15 locomotion record is unchanged; Zantetsuken is unchanged from v15.")
    log("The MSET directory contains one appended record; no remastered folder is used.")
